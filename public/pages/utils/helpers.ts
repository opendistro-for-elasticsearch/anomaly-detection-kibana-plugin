...
export function getVisibleOptions(indices: CatIndex[], aliases: IndexAlias[], maxLen: number) {
  ...
  if (visibleIndices.length + visibleAliases.length > maxLen) {
    if (visibleIndices.length >= maxLen) {
      ...
      truncatedAliases = visibleAliases.slice(0, maxLen - visibleIndices.length);
    }
  }
  return [
    {
